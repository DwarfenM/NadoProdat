    ...
    override fun bindIntents() {
        ...
        val productSaveIntent: Observable<SingleProductState> =
            intent(SingleProductView::saveProductIntent).flatMap {prod ->
                if(prod.isToCreate) {
                    npDb.productDao().insertProduct(Product(name = prod.productName, count = prod.productCount.toInt(), salesPrice = prod.productSalesPrice,costPrice = prod.productCostPrice, crDate = System.currentTimeMillis())).toObservable().flatMap {id ->
                        for((key, value) in prod.props) {
                            npDb.propertyDao().insertProperty(key).flatMap {
                                npDb.propertyValuesDao().insertPropertyValues(PropertyValues(productId = id, propertyId = it,value = value))
                            }.subscribe()
                        }
        ...
        val allIntents = Observable.merge(receivePropertiesIntent, productSaveIntent).observeOn(AndroidSchedulers.mainThread())
